<?php
	...
	if(isset($_POST['Add'])) {
		...
		$result = $user->addsignatory($setfirstname, $setlastname, $setmiddleI, $setinitials, $setposition, $checkgisce, $checkgl);
                ...
                <div class="container-fluid">
                    ...
                    <a class="nav-link toggle tohover" data-id="<?php echo $_SESSION['userId'];?>" data-target="#userAccount" style="margin-left: 10px;" data-toggle="modal" aria-haspopup="true" aria-expanded="false">
                        <?php $name = explode(' ',$_SESSION['userfullname']); $namef=strtoupper($name[0]); echo $namef;?>
										...
										<a class="nav-link toggle tohover" href="#" data-target="#AddSignatory"  style="border-left: solid 4px gray" data-toggle="modal" aria-haspopup="true" aria-expanded="false">Add Signatory<a>          <button class="btn btn-dark d-inline-block d-lg-none ml-auto" type="button" data-toggle="collapse" data-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
            ...
            <div class="container-fluid"  style="padding-left: 5%">
                <div class="table-responsive-lg">
									...
									<table id="admintable" class="table table-fixed table-striped table-hover highlight responsive-table" style="width: 100%; margin: 2% 0% 0% 0%;">
										<thead>
										<tr>
											<th scope="col" style="width: 15%">Signatory id</th>
											<th scope="col">Signatory Name</th>
											<th scope="col">Signatory Position</th>
										...
										if(!isset($_REQUEST['buttonSearch'])){
											$getuser = $user->get_signatory_to_admin_table();
											if($getuser){
												...
												echo "<tr>
															...
															<button type='button' name='view' class='btn btn-primary deep-sky' data-toggle='modal' data-id='" . $value["signatory_id"]. "' style='margin-right: 10px;' data-target='#UpdateSignatory'> Update </button>
															</td>
															</tr>
														";		
												}
											...
											$getuser = $user->search_signatory($search);
											if($getuser){
												foreach($getuser as $index => $value){
												echo "<tr>
															<td scope='row' style='width: 15%'>" . $value["signatory_id"] . "</td> 
...
<div class="module">
    ...
    <script>
		...
		<div class="modal hide fade" id="SignatoryInfo" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" data-backdrop="static" data-keyboard="false">
			<div class="modal-dialog modal-md" role="document">
				<div class="modal-content">
					<div class="modal-header">
            			<h5 class="modal-title" id="exampleModalLabel">Show Signatory Information</h5>
						<button type="button" class="close" data-dismiss="modal" aria-label="Close">
							<span aria-hidden="true">&times;</span>
						</button>
					</div>
		...
		<div class="modal hide fade" id="UpdateSignatory" tabindex="-1" role="dialog" aria-labelledby="ModalLabel" aria-hidden="true" data-backdrop="static" data-keyboard="false">
			<div class="modal-dialog modal-md" role="document">
				<div class="modal-content">
					<div class="modal-header">
							<h5 class="modal-title" id="exampleModalLabel">Update Signatory Information</h5>
							<button type="button" class="close" data-dismiss="modal" aria-label="Close">
								<span aria-hidden="true">&times;</span>
							</button>
					</div>
	...
	<script type="text/javascript">
	$('#SignatoryInfo').appendTo("body").on('show.bs.modal', function (event) {
						var button = $(event.relatedTarget) // Button that triggered the modal
	...
	$('#UpdateSignatory').appendTo("body").on('show.bs.modal', function (event) {
						var button = $(event.relatedTarget) // Button that triggered the modal
						var empid = button.data('id') // Extract info from data-* attributes
						var modal = $(this);
						var dataString = 'id=' + empid;
...
<div class="modal hide fade" id="AddSignatory" tabindex="-1" role="dialog" aria-labelledby="ModalLabel" aria-hidden="true" data-backdrop="static" data-keyboard="false">
	<div class="modal-dialog modal-md" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="exampleModalLabel">Add Signatory Information</h5>
					<button type="button" class="close" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">&times;</span>
					</button>
				</div>
	...
	<script type="text/javascript">
		$('#AddSignatory').appendTo("body").on('show.bs.modal', function (event) {
			var button = $(event.relatedTarget) // Button that triggered the modal
			var modal = $(this);
