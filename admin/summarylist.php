<?php
    ...
    if(!empty($_POST["d_1"]) && !empty($_POST["d_2"]) && !empty($_POST['emp'])){
        ...
        $date2 = $_POST["d_2"];
        $emp = $_POST['emp'];
        $numrows = $user->getClientAndEmpSetNum($emp, $date1, $date2);
    }else{
        $date1="";
        $date2="";
        $emp="";
        $numrows = $user->getClientAndEmpNum($datenow, $datenow2);
    }
        ...
        <div id="content">
            ...
            <div class="container-fluid"  style="padding-left: 5%">
                <div class="table-responsive-lg">
					<h5>Summary List</h5>
                    <form class="form-group" action="summarylist.php" method="POST">
                        ...
                        <div class="row" style="padding-bottom: 20px;">
                            ...
                            <div class="col-lg-2">Employee:
                                <select id="emp" name="emp" class="form-control border border-primary" type="text" required>
                                    ...
                                    <?php 
                                        $encoder = $user->encodersummarylist();
                                        $socwork = $user->swsummarylist();
                                        if (!empty($encoder)) {
                                            foreach ($encoder as $index => $value) {
                                                echo '<option value="'.$value['encoded_encoder'].'">'.$user->getEncoder($value['encoded_encoder']).'</option>';
                                            }
                                        }
                                        if (!empty($socwork)) {
                                            foreach ($socwork as $index => $value) {
                                                echo '<option value="'.$value['encoded_socialWork'].'">'.$user->getsocialWork($value['encoded_socialWork']).'</option>';
                                            }
                                        }
                                    ?>
					...
					<table id="admintable" class="table table-fixed table-striped table-hover highlight responsive-table" style="width: 100%; margin: 2% 0% 0% 0%;">
						<thead>
							...
							<th scope="col" style="width: 15%">Date Accomplished</th>
							</tr>
						</thead>
						<tbody>
							<?php
                            if (!empty($emp) && !empty($date1) && !empty($date2)) {
                                $get = $user->getClientAndEmpSet($emp,$date1,$date2);
                                if ($get) {
                                    foreach ($get as $index => $value) {
                                        $dateaccomplished = date('F j, Y g:i A', strtotime($value['date_accomplished']));
                                        echo "<tr>
                                        ...
                                        echo "</td>
                                        <td>" . $user->getEncoder($value["encoded_encoder"]) . "</td>
                                        <td>" . $user->getsocialWork($value["encoded_socialWork"]) . "</td>
                                ...
                                $get = $user->getClientAndEmp($datenow, $datenow2);
                                if ($get) {
                                    foreach ($get as $index => $value) {
                                        $dateaccomplished = date('F j, Y g:i A', strtotime($value['date_accomplished']));
                                        echo "<tr>
