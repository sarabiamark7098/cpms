<?php
	...
	class User{
		...
		$query = "";
			...
			$query .= "INSERT INTO assistance (trans_id, type, amount, mode, fund, purpose, type_description) 
					VALUES ('{$trans_id}', '{$type1}', '{$a1}', '{$m1}', '', '{$pur1}', 'Type1')";
					if($type2 !=""){
			...
			if($m1 == "GL"){ 
				...
				if($amountcon < 20001){ $query .= ", signatory_GL = '{$sign_id}'"; }
			}
			if($m2 == "GL"){ 
				$amountcon2 = str_replace(",","", $a2);
				if($amountcon2 < 20001){ $query .= ", signatory_GL = '{$sign_id}'"; }
			} 
		...
		public function updateGIS($empid, $trans_id, $id, $p1, $p2, $p3, $e1, $e2, $e3, $t1, $t2, $t3, $b1, $b2, $b3, $s1, $s2, $s3, $s4, $ref_name,
			...
			if($m1 == "GL"){ 
				$amountcon = str_replace(",","", $a1);
				if($amountcon < 20001){ $query .= ", signatory_GL = '{$sign_id}'"; }
				elseif($amountcon >= 20001){ $query .= ", signatory_GL = ''"; }
			}
			if($m2 == "GL"){ 
				$amountcon2 = str_replace(",","", $a2);
				if($amountcon2 < 20001){ $query .= ", signatory_GL = '{$sign_id}'"; }
				elseif($amountcon2 >= 20001){ $query .= ", signatory_GL = ''"; }
			}
			$query .= " WHERE trans_id = '{$trans_id}';";
			$result = mysqli_multi_query($this->db, $query);
		...
		public function getSignatoryFullname($id){
			...
			$result = mysqli_query($this->db,$query);
			$row = mysqli_fetch_assoc($result);
			$data = $row['first_name'] ." ". (empty($row['middle_I'])?"":$row['middle_I'].". "). $row['last_name'] ."-". $row['position']."-". $id;
			if($data){
				return $data;
			}
			else {
		...
		public function insertCOE($id, $docu, $id_pres, $signName, $others_input, $amount1, $amount2, $amount3, $amount4, $amount5, $am, $mode, $id_sign){
			...
			$query = "INSERT INTO coe (trans_id, document, id_presented, others_input) 
						VALUES 
						('{$id}','{$docu}','{$id_pres}','{$others_input}');";
			if($am >= 20001 && $mode == "GL"){
		...
		public function updateCOE($id, $docu, $id_pres, $signName, $others_input, $amount1, $amount2, $amount3, $amount4, $amount5, $am, $mode, $id_sign){
			$others_input = mysqli_real_escape_string($this->db,$others_input);
			$signid = 0;
			...
			$query = "UPDATE coe SET document='{$docu}', id_presented='{$id_pres}', others_input='{$others_input}' where trans_id = '{$id}';";
			if($am >= 20001 && $mode == "GL"){
				$query .= "UPDATE tbl_transaction SET signatory_GL = '{$signid}' WHERE trans_id = '{$id}';";
			}if($am < 20001){
	...
	public function controlNumberForGL(){
		...
		// $result = mysqli_query($this->db, $query);
		// $row = mysqli_fetch_assoc($result);
		$glid = str_pad($count, 6, 0, STR_PAD_LEFT);
