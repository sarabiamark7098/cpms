<?php 
    ...
    if(isset($_GET['id'])){
        ...
        $GISsignatory= $user->getsignatory($client['signatory_id']); //kwaun ang data sa signatory using sign_id 
        $GISsignatoryName = strtoupper($GISsignatory['first_name'] ." ". (!empty($GISsignatory['middle_I'])?$GISsignatory['middle_I'] .". ":""). $GISsignatory['last_name']);
...
<html>
                            ...
                            <div class="card-body">
                                ...
                                <?php
                                    if(substr_count(strval($type), "Medic") > 0){
                                        ...
                                        <div class="row">
                                            <div><input type="checkbox" class="lg" name="others_m" id="othersM" value="OTHERS MEDICAL" '.$user->checkCheck($record['document'], "", "OTHERS MEDICAL").'> Others
                                            <input type="text" class="lg" name="others_medical" id="others_medical" value="'. $record['others_medical'] .'">
                                            </div>
                                        </div>
                                        ';
                                        ...
                                        <div class="row">
                                            <div><input type="checkbox" class="lg" name="others_b" id="othersB" value="OTHERS BURIAL" '.$user->checkCheck($record['document'], "", "OTHERS BURIAL").'> Others
                                            <input type="text" class="lg" name="others_burial" id="others_medical" value="'. $record['others_burial'] .'">
                                            </div>
                                        </div>
                                        ';
                                    }
    ...
    if(isset($_POST["save"])){
        ...
        $user->insertCOE($_GET['id'], $docu, $id_pres, $signName, $others_input, $others_medical, $others_burial, $amount1, $amount2, $amount3, $amount4, $amount5, $am, $modecon, $id_sign);
    }
    ...
    if(isset($_POST['update'])){
        ...
        if(isset($_POST['others_m'])){ //if na check si other
            $others_medical = mysqli_real_escape_string($user->db, trim(strtoupper(trim($_POST['others_medical']))));
        } 
        
        if(isset($_POST['others_b'])){ //if na check si other
            $others_burial = mysqli_real_escape_string($user->db, trim(strtoupper(trim($_POST['others_burial']))));
        }
        ...
        $modecon = $client_assistance[1]['mode'];
        // echo $signName;
        $user->updateCOE($_GET['id'], $docu, $id_pres, $signName, $others_input, $others_medical, $others_burial, $amount1, $amount2, $amount3, $amount4, $amount5, $am, $modecon, $id_sign);
    }
?>
    <script>
