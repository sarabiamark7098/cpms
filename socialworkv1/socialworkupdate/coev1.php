<?php 
    ...
    if(isset($_GET['id'])){
        ...
        $GISsignatoryName = strtoupper((!empty($GISsignatory['name_title'])?($GISsignatory['name_title'] != " "?$GISsignatory['name_title'] ." ":""):"")). strtoupper($GISsignatory['first_name'] ." ". (!empty($GISsignatory['middle_I'])?($GISsignatory['middle_I'] != " "?$GISsignatory['middle_I'] .". ":""):""). $GISsignatory['last_name']);
        ...
        if($record){
            ...
            $COEsignatoryName = strtoupper((!empty($COEsignatory['name_title'])?($COEsignatory['name_title'] != " "?$COEsignatory['name_title'] ." ":""):"")). strtoupper($COEsignatory['first_name'] ." ". $COEsignatory['middle_I'] .". ". $COEsignatory['last_name']);
        }

        $soc_worker = $user->getuserInfo($client['encoded_socialWork']); //get soc-worker data from database
...
<html>
                            ...
                            <div class="card-body">
                                ...
                                <?php
                                    if(substr_count(strval($type), "Medic") > 0){
                                        ...
                                        echo '
                                        <div class="row">
                                            <div><input type="checkbox" class="lg" name="regD" id="regD" value="REGISTERED DEATH CERTIFICATE" '.$user->checkCheck($record['document'], "", "DEATH CERTIFICATE").'> Registered Death Certificate</div>
                                        </div>
                                        <div class="row">
    ...
    if(isset($_POST["save"])){
        ...
        $user->insertCOE($_GET['id'], $docu, $id_pres, $signName, $others_input, $amount1, $amount2, $amount3, $amount4, $amount5, $am, $modecon);
    }
    ...
    if(isset($_POST['update'])){
		...
		if(!empty($_POST['amountf1'])){
            $amount1 = $_POST['amountf1'];
        }
        if(!empty($_POST['amountf11'])){
            $amount2 = $_POST['amountf11'];
        }
        if(!empty($_POST['amountf111'])){
            $amount3 = $_POST['amountf111'];
        }
        if(!empty($_POST['amountf1111'])){
            $amount4 = $_POST['amountf1111'];
        }
        if(!empty($_POST['amountf11111'])){
            $amount5 = $_POST['amountf11111'];
        }
        $docu=mysqli_real_escape_string($user->db,$docu);
        $modecon = $client_assistance[1]['mode'];
        ...
        // echo $signName;
        $user->updateCOE($_GET['id'], $docu, $id_pres, $signName, $others_input, $amount1, $amount2, $amount3, $amount4, $amount5, $am, $modecon, $gis['signatory_id']);
    }
?>
    <script>
        ...
        $(document).ready(function() {
            ...
            var total3 = Number(0.00);
            var total4 = Number(0.00);
            var total5 = Number(0.00);
