<?php 
    ...
    if(isset($_GET['id'])){
        ...
        $GISsignatoryName = strtoupper($GISsignatory['first_name'] ." ". (!empty($GISsignatory['middle_I'])?($GISsignatory['middle_I'] != " "?$GISsignatory['middle_I'] .". ":""):""). $GISsignatory['last_name']);
        ...
        if($record){
            ...
            $COEsignatoryName = strtoupper($COEsignatory['first_name'] ." ". (!empty($COEsignatory['middle_I'])?$COEsignatory['middle_I'] .". ":""). $COEsignatory['last_name']);
        }
...
<html>
                ...
                <?php if($am >= $rec_amount || $am2 >= $rec_amount){ ?>
                    <div class="row">
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                Amount is more than Twenty Thousand Pesos (Php 20,000).
                                <div>
                                <label> Approved By : </label>&nbsp&nbsp&nbsp
                                    <input style="text-transform: uppercase; width:50%" id="coesignatoryid" list="coesign" name="coesignName" value="<?php echo empty($client['signatory_GL'])?"":$user->getSignatoryFullname($client['signatory_GL']) ?>" required>
                                    <datalist id="coesign">
                                        <?php 
    ...
    if(isset($_POST["save"])){
        ...
        $user->insertCOE($_GET['id'], $docu, $id_pres, $signName, $others_input, $amount1, $amount2, $amount3, $amount4, $amount5, $am, $modecon, $id_sign);
    }
    ...
    if(isset($_POST['update'])){
        ...
        $modecon = $client_assistance[1]['mode'];
        // echo $signName;
        $user->updateCOE($_GET['id'], $docu, $id_pres, $signName, $others_input, $amount1, $amount2, $amount3, $amount4, $amount5, $am, $modecon, $id_sign);
    }
?>
    <script>
