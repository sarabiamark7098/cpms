<?php 
    ...
    $dateexpiredcreatenew = date("F j, Y");
    ...
    if(isset($_POST['done'])){
        $execute = $user->reissueexpired($_GET['id']);
        if($execute){
...
<!-- Modal for Updating Amount -->
	...
	<script>
        ...
        function printPCV(){
            var divElements = document.getElementById('pcv').innerHTML;
            //Get the HTML of whole page
            var oldPage = document.body.innerHTML;
            //Reset the page's HTML with div's HTML only
            document.body.innerHTML = 
    ...
    <div class="container">
        ...
        <div class="form-group row">
            <div class="card border-info mb3" style="width:100%">
                <h4 class="card-header text-success">Reissueance on Expired Guarantee Letter</h4>
                <div class="card-body">
                <form class="form-group" action="reissuanceexpired.php?id=<?php echo $id ?>" method="POST" autocomplete="off">
                    <div class="row text-center" >
                        ...
                        <div class="col"> <button class="btn btn-<?php echo empty($cash)?"default":"success" ?> btn-block no-print" onclick="printPCV()" <?php echo empty($cash)?"disabled":"" ?>>CASH</button></div>
                    </div>
                    <br>
                    <br>
                    <div class="row text-center">
                        <div class="col"></div>
                        <div class="col"><button class="btn btn-success btn-block no-print " name="done" id="done">Done</button></div>
                        <div class="col"></div>
                    </div>
                </form>
                </div>
...
<div>
    <div id="pcv" hidden>
        <?php 
            if(($client['mode1'] == "PCV" || $client['mode2'] == "PCV")){
                include('cash.php');
            }
        ?>
    </div>
