    ...
    <div class="container" style="font-size: 20px">
        ...
        <div class="row"> 
            ...
            <div class="col-5 ml-auto text-right">
                <p>Control No: <?php echo strtoupper($client_assistance[1]['fund']);?>-<span id="number"><?php echo $gl['control_no']?></span></p>
        ...
        <div class="justify">
            <?php   
                if(strtolower($client['relation']) == "self"){
                    ...
                    <b>'. $user->toWord($client_assistance[1]["amount"]) .' (Php'. $client_assistance[1]['amount'].')</b> requested by <b>'.strtoupper($coename).'</b>. 
                    This is intended for '. (strtolower($client['sex'])=='male'?'his': 'her') .' <b>'. strtoupper($client_assistance[1]['type']) .'</b>  from '. strtoupper($address['client']) .'.</p>';
                }else{
                    echo '<p class="first">The Department of Social Welfare and Development (DSWD) guarantees to pay the amount of 
                    <b>'. $user->toWord($client_assistance[1]["amount"]) .' (Php '. $client_assistance[1]['amount'] .')</b> requested by <b>'.strtoupper($coename).'</b>. 
                    This is intended for the <b>'. strtoupper($client_assistance[1]['type']) .'</b> of <b>'.$coebname.'</b> from '. strtoupper($address['beneficiary']) .'.</p>';
                }
            ?>
        </div>
